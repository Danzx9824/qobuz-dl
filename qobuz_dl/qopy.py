...
class Client:
    def __init__(self, email_or_userid, pwd_or_token, app_id, secrets):
        ...
        self.sec = None
        self.auth(email_or_userid, pwd_or_token)
        self.cfg_setup()

    def api_call(self, epoint, **kwargs):
        if epoint == "user/login":
            params = {
                "email": kwargs["email_or_userid"],
                "password": kwargs["pwd_or_token"],
                "app_id": self.id,
            } \
            if '@' in kwargs["email_or_userid"] else {
                "user_id": kwargs["email_or_userid"],
                "user_auth_token": kwargs["pwd_or_token"],
                "app_id": self.id,
            }
    ...
    def auth(self, email_or_userid, pwd_or_token):
        usr_info = self.api_call("user/login", email_or_userid=email_or_userid, pwd_or_token=pwd_or_token)
